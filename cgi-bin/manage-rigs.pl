...
my $db_path = "/opt/netrig/data/rigs.db";
my $db_sql = "/opt/netrig/sql/rigs.sql";
...
# XXX: We should instead check if netrig_rigs table exists
if ($db_empty) {
...
if ($frm_act eq "get") {
   ...
   my $sql = "SELECT * FROM netrig_rigs;";
   my $res = $dbh->do($sql);
   my $rows = $dbh->rows();
