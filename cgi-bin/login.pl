...
open my $log, '>>', "/opt/netrig/tmp/login.log" or die "Unable to open tmp/login.log: $!\n";
...
# users database
my $db_path = "/opt/netrig/data/users.db";
my $db_sql = "/opt/netrig/sql/users.sql";
...
# session database
my $sess_path = "/opt/netrig/data/users.db";
my $sess_sql = "/opt/netrig/sql/user.sql";
...
# XXX: We should instead check if netrig_userss table exists
if ($db_empty) {
...
sub add_sip_user {
   ...
   my $pjsip_file = "/opt/netrig/etc/asterisk/netrig/user.$sip_user.conf";
...
my $query = "SELECT callsign, privileges, disabled FROM netrig_users WHERE callsign = ? and password = ?;";
