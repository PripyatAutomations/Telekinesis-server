#!/usr/bin/perl
# Here we provide the ARI bridge for a single radio...
use strict;
use warnings;
use Scalar::Util;
...
my $app_name = "netrig-ari";
my $VERSION = "20231111.01";
...
# Logger #
##########
my $log_file = "/opt/netrig/logs/netrig-ssb-ari.$radio.log";
...
open (our $log_fh, '>>', $log_file) or die("Cant open log file $log_file for writing\n");
...
#   # Capture the log_type and level from arguments
   ...
   $log_msg = $datestamp . " [$log_type/$lvl]";
...
# cli interface
my $socket = '/opt/netrig/run/ari.sock';

my $ari_pass_file = "/opt/netrig/run/ari.$radio.pass";
open (our $ari_pass_fh, '<', $ari_pass_file) or die("Cannot open $ari_pass_file for reading\n");
my $ari_apikey = '';
while (1) {
   my $res = read $ari_pass_fh, $ari_apikey, 512, length($ari_apikey);
   die "$!\n" if not defined $res;
   last if not $res;
}
chomp $ari_apikey;
my ($ari_user, $ari_pass) = (split /:/, $ari_apikey)[0, 1];
close($ari_pass_fh);

Log "core", $LOG_INFO, "ari_apikey: $ari_apikey";

# Here we need to be smart about multi-radios...
my $ari = {
   "host" => "127.0.0.1",
   "port" => 8088,
   "prefix" => "/pbx/ari",
...
###############################################
print "netrig-ssb-ari.pl version <$VERSION>\n";
...
Log "hamlib", $LOG_INFO, "Initializing hamlib interface to rig $radio";
...
if ($ptt) {
   Log "ptt", $LOG_BUG, "Clearing $radio PTT...";
...
if (defined $radio0->{'hamlib_riginfo'}) {
...
sub ari_speech {
   my $chan_id = $_[0];
   my $prompt = $_[1];
   ari_play($chan_id, "sound:netrig/" . $client->{'tts_voice'} . "/" . $prompt);
}
...
sub start_baresip_channel {
   ...
   my $file = "/opt/netrig/logs/baresip-$chan_name.log";
   open my $fh, '>', $file;
   defined(my $pid = fork) or die "fork: $!";
   if (!$pid) {
      open STDOUT, '>&', $fh;
      exec("baresip", ("baresip", "-f", "/opt/netrig/tmp/baresip-$radio", "-e", "$dialstring", "-d"));
   }
   waitpid $pid, 0;
   my $retcode = $?;
...
sub station_modeset {
   ...
   if ($new_mode eq "phone") {
      ...
      system("/opt/netrig/bin/modeset-off");
      system("/opt/netrig/genconf/baresip-ua");
      start_baresip_channel($radio);
   } elsif ($new_mode eq "digi") {
      Log "station", $LOG_AUDIT, "Switching to DIGI mode";
      system("/opt/netrig/bin/modeset-digi");
   } elsif ($new_mode eq "winlink") {
      Log "station", $LOG_AUDIT, "Switching to WINLINK mode";
      system("/opt/netrig/bin/modeset-winlink");
   }
}
      ...
      if ($digit eq '*') {
         ...
         if ($rfside) {
            ...
            if ($duration >= 1000) {
               ...
               Log "ptt", $LOG_AUDIT, "Toggling PTT blocked status on $radio to: " . ($blocked ? "true" : "false");
         ...
         if ($rfside) {
            ...
            if ($rdigits eq '#') {
               if ($radio0->{'using_vox'}) {
                  if ($muted) {
                     $muted = 0;
                     Log "dtmf", $LOG_AUDIT, "$radio MUTE off";
                     ari_speech($chan_id, "mute_off");
                  } else {
                     $muted = 1;
                     Log "dtmf", $LOG_AUDIT, "$radio MUTE on";
                 ...
                 if ($radio0->{'ptt_active'}) {
                    $rig->set_ptt($radio0->{'active_vfo'}, $Hamlib::RIG_PTT_OFF);
                    $radio0->{'ptt_active'} = 0;
                    Log "dtmf", $LOG_AUDIT, "$radio PTT OFF";
                    ari_play($chan_id, "sound:beep");
                 } else {
                    if (!($radio0->{'ptt_blocked'})) {
                       $rig->set_ptt($radio0->{'active_vfo'}, $Hamlib::RIG_PTT_ON_DATA);
                       $radio0->{'ptt_active'} = 1;
                       Log "dtmf", $LOG_AUDIT, "$radio PTT ON";
                       ari_play($chan_id, "sound:beep");
                    } else {
                       Log "ptt", $LOG_AUDIT, "PTT for $radio is blocked, igoring PTT ON request";
...
$ari_conn->connect(
   ...
   on_connected => sub {
      ...
      my $bridge = ari_bridge_find_or_create($radio);
      ...
      if ($bridge eq "") {
         $bridge = ari_bridge_find_or_create($radio);
      }
   ...
   url => "ws://" . $ari->{'host'} . ":" . $ari->{'port'} . $ari->{'prefix'} .
          "/events?api_key=$ari_apikey&app=$app_name"
)->then(sub {
