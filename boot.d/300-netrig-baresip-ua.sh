...
if [ $? -ne 0 ]; then
   ...
   adduser --system \
   	   --comment "baresip user for netrig" \
...
# XXX: multiradio
echo "* Configuring..."
/opt/netrig/genconf/baresip-backend.pl ${RADIO}

echo "* Fixing permissions..."

echo "* Starting baresip ua..."
#(sudo -u ${BARESIP_USER} env -i /opt/netrig/run/baresip-${RADIO}/launch.sh) &
sudo -u ${BARESIP_USER} /opt/netrig/run/baresip-${RADIO}/launch.sh &
