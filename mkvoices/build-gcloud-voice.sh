...
gcloud iam service-accounts keys create /opt/netrig/etc/gcloud/sa-private-key.json  --iam-account=SA_NAME @PROJECT_ID.iam.gserviceaccount.com
...
for template in langs/${TEMPLATE_LOCALE}/*.ssml; do
   ...
   if [ ! -f "${DEST}/${OFILE}" ]; then
      ...
      FULLTEXT="$(cat /opt/netrig/voices/${template})"
